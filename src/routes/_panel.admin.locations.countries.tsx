...
import { z } from "zod";
import { Plus, Globe, Pencil, Trash2, MoreHorizontal } from "lucide-react";
...
function CountriesPage() {
  ...
  if (!has("locations.view")) {
    return (
      <div className="p-6">
        <PageHeader
          title={t("admin.locations.countries.title")}
        ...
        actions={
          <Can perm="locations.update">
            <Button
              ...
              onClick={openNew}
            >
              <Plus className="me-1.5 h-4 w-4" /> {t("admin.locations.countries.new")}
            </Button>
          </Can>
        }
      />
        ...
        <PageStates
          ...
          empty={
            <div className="rounded-2xl border border-dashed bg-muted/30 p-16 text-center text-sm text-muted-foreground">
              {t("admin.locations.countries.emptyTitle")}
            </div>
          }
        >
          <DataTable
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="w-full sm:max-w-md">
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex h-full flex-col">
            <SheetHeader>
              <SheetTitle>
                {editing ? t("admin.common.edit") : t("admin.locations.countries.new")}
              </SheetTitle>
