...
export const Route = createFileRoute("/_panel/admin/locations/cities")({
  head: () => ({ meta: [{ title: "Cities — Mixlebs Admin" }] }),
...
function CitiesPage() {
  ...
  const cities = useMemo(
    () => (citiesQuery.data?.results ?? []).map(toAdminCity),
    [citiesQuery.data],
  );
  ...
  if (!has("locations.view")) {
    return (
      <div className="p-6">
        <PageHeader
          title={t("admin.locations.cities.title")}
  ...
  function openNew() {
    setEditing(null);
    form.reset({ name_en: "", name_ar: "", code: "", region: "" });
    setOpen(true);
  }
  ...
  async function onSubmit(values: Values) {
    ...
    try {
      if (editing) {
        await updateCity({ data: { id: Number(editing.id), body } });
      } else {
        await createCity({ data: body });
      }
  ...
  async function removeCity(c: AdminCity) {
    try {
      await deleteCity({ data: { id: Number(c.id) } });
      await queryClient.invalidateQueries({ queryKey: ["cities"] });
  ...
  const columns: Column<AdminCity>[] = [
    ...
    {
      id: "region",
      header: t("admin.locations.colRegion"),
      cell: (c) => (
  ...
  return (
    <div className="p-6">
        ...
        actions={
          <Can perm="locations.update">
            <Button
              ...
              onClick={openNew}
            >
              <Plus className="me-1.5 h-4 w-4" /> {t("admin.locations.cities.new")}
            </Button>
          </Can>
        }
      />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <KpiCard
          label={t("admin.locations.cities.title")}
          value={cities.length}
        ...
        <KpiCard
          ...
          value={new Set(cities.map((c) => c.region)).size}
        />
      </div>
        ...
        <PageStates
          ...
          empty={
            <div className="rounded-2xl border border-dashed bg-muted/30 p-16 text-center text-sm text-muted-foreground">
              {t("admin.locations.cities.emptyTitle")}
            </div>
          }
        >
          <DataTable
                ...
                ? (c) => (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <ConfirmDialog
                          destructive
                          title={t("admin.locations.cities.deleteTitle")}
                          confirmLabel={t("admin.common.delete")}
                          typeToConfirm={c.code}
                          onConfirm={() => removeCity(c)}
                          trigger={
                            <DropdownMenuItem
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="w-full sm:max-w-md">
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex h-full flex-col">
            <SheetHeader>
              <SheetTitle>
                {editing ? t("admin.common.edit") : t("admin.locations.cities.new")}
              </SheetTitle>
